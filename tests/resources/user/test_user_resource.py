...
from exceptions import UserAlreadyExists, InvalidInputError
...
class TestUser:
    def test_create_user_ok(self, client, db_session) -> None:
        ...
        response = client.post(f"/api/{os.getenv('API_VERSION', 'v1')}/users", json=body)
    ...
    def test_create_user_with_exsisting_email(self, client) -> None:
        body = {"email": "myuser@sample.com", "password": "encoded"}
        client.post(f"/api/{os.getenv('API_VERSION', 'v1')}/users", json=body)
        response = client.post(f"/api/{os.getenv('API_VERSION', 'v1')}/users", json=body)
    ...
    def test_get_user_ok(self, client, db_session) -> None:
        ...
        get_user_response = client.get(f"/api/{os.getenv('API_VERSION', 'v1')}/users?email=myuser@sample.com")
    ...
    def test_get_user_not_found(self, client) -> None:
        response = client.get(f"/api/{os.getenv('API_VERSION', 'v1')}/users?email=notfoundemail@sample.com")
